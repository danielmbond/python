...
for root, dirs, files in os.walk(SABNZBD_COMPLETE_DIR):
    ...
    for name in files:
        ...
        if "UNPACK" not in file:
            if(not file.endswith(".mp4") and not
               file.endswith(".m4v") or fileSize < 20):
                print("Deleting", name, fileSize)
                os.remove(file)
                continue
            else:
                try:
                    os.rename(file, os.path.join(SABNZBD_COMPLETE_DIR,
                                                 directoryToFileName(file)))
                    print("RENAME", file, os.path.join(
                        SABNZBD_COMPLETE_DIR, directoryToFileName(file)))
